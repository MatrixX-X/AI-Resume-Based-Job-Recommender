...
# 1. Import the function that gets your main, cached embedding model
from utils.embeddings import get_embedding_model

# 2. Use Streamlit's caching to initialize KeyBERT only once
@st.cache_resource
def get_keybert_model():
    """
    Initializes KeyBERT with a pre-loaded SentenceTransformer model.
    """
    # Get the SentenceTransformer model that is already cached
    embedding_model = get_embedding_model()
    
    # Pass the loaded model to KeyBERT
...
def extract_skills(text, top_n=10):
    ...
    keywords = get_keybert_model().extract_keywords(text, keyphrase_ngram_range=(1, 2), stop_words='english', top_n=top_n)
