...
from utils.embeddings import generate_embedding, get_embedding_model, store_embedding
...
# st.title("Secret Debugging Info")
# st.write("All available secret keys loaded by the app:")
...
st.write("DEBUG QDRANT_URL:", os.getenv("QDRANT_URL"))
st.write("DEBUG QDRANT_API_KEY set?", bool(os.getenv("QDRANT_API_KEY")))
...
if uploaded_file is not None:
    # st.write("--- DEBUG: Checkpoint 1: File upload detected. ---")
    
    # 1. Create a temporary file to read from
    with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as tmp:
        tmp.write(uploaded_file.getvalue())
        file_path = tmp.name
    
    # st.write(f"--- DEBUG: Checkpoint 2: Temp file created at {file_path} ---")

    # 2. Parse the text from the PDF
    resume_text = extract_text_from_pdf(file_path)
    
    if resume_text:
        # st.write("--- DEBUG: Checkpoint 3: PDF text extracted successfully. ---")
        
        # 3. Store the embedding (this checks for duplicates)
        with st.spinner("Storing resume embedding..."):
            # st.write("--- DEBUG: Checkpoint 4: Calling store_embedding... ---")
            is_new, resume_id = store_embedding(resume_text)
            # st.write(f"--- DEBUG: Checkpoint 5: store_embedding finished. New: {is_new}, ID: {resume_id} ---")
        
        # 4. Now, display the results
        # st.write("--- DEBUG: Checkpoint 6: Calling process_resume_and_display_results... ---")
        process_resume_and_display_results(file_path, collection_name)
        st.write("--- DEBUG: Checkpoint 7: process_resume_and_display_results finished. ---")

    # else:
    #     st.error("--- DEBUG: FAILED at Checkpoint 3. Could not extract text from the uploaded resume. ---")

    # 5. Clean up the temporary file
    os.remove(file_path)
    # st.write("--- DEBUG: Checkpoint 8: Temp file removed. Process finished. ---")

elif selected_resume != "Choose a sample...":
    # This part for sample resumes can remain the same
    # st.write("--- DEBUG: Checkpoint 9: sample resume. ---")
